...
function getRow(row) { // 데이터 한건 처리.
    ...
    tr.onclick = function(e) {
        ...
        document.getElementById('empId').value = 
            this.childNodes[0].innerHTML; // id값
        document.getElementById('name').value = 
            this.childNodes[1].innerHTML; // name값
        document.getElementById('email').value = 
            this.childNodes[2].innerHTML; // email값
        document.getElementById('hire').value = 
            this.childNodes[3].innerHTML; // hire값
        document.getElementById('job').value = 
            this.childNodes[4].innerHTML; // job값
    ...
    btn.onclick = function(e) {
        e.stopPropagation(); // button < td < tr < table < body   상위로가는 이벤트전달 차단
